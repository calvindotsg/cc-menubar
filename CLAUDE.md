...
- **Cold-start-safe terminal spawning from SwiftBar plugins** (`ccusage_helper.sh`, `render.py`): for SwiftBar menu items that open a terminal window and run a compound command, emit `bash="<helper>" terminal=false` in the render DSL (**double-quoted path is mandatory** — SwiftBar's unquoted-value parser in `MenuLineParameters.swift::getParams` splits at the first whitespace and does not honor `\ ` escapes, so a backslash-escaped path under `~/Library/Application Support/` silently truncates to `/Users/x/Library/Application\` and the click does nothing). From the helper shell out with `exec open -na Ghostty.app --args -e /bin/bash -c "<compound>"` (argv-split form — `/usr/bin/login` exec's `/bin/bash` as a real binary and bash parses the compound). Keep Terminal.app on a `mktemp` + `open -a Terminal.app "$tmpf"` fallback; its file-handler has a shell-ready handshake (Ghostty's does not — Ghostty.app's Info.plist registers `Terminal scripts` with `LSHandlerRank=Alternate` and types the script path into the PTY as keyboard input without handshake, which races on cold start). Detect Ghostty via `[ -d /Applications/Ghostty.app ]`, never `shutil.which("ghostty")` — cmux ships a bundled `ghostty` binary on PATH that returns a false positive. Anti-patterns: backslash-escaped spaces in `bash=<path>` (silent click failure), bare `open -a Ghostty.app <script>` (v2.0.0 cold-start race), single-string `open -na Ghostty.app --args -e "cmd1; cmd2"` (login tries to exec the compound as a filename and fails). Non-discoverable from code: all three helper-invocation forms look nearly identical and the DSL-quoting rule is not explicit in SwiftBar's README; only empirical cold + warm testing against Ghostty 1.3.1 and reading `MenuLineParameters.swift` reveal which combination works.
